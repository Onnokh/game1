// Combined shadow and foliage sway shader fragment shader
// Applies foliage sway effect and then makes it black for shadow
...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    // Apply foliage sway effect (same as foliage_sway_shader)
    ...
    // Sample texture with swayed UVs
    vec4 swayedColor = Texel(texture, result_uvs);
    
    // Apply shadow effect: make it black with shadow opacity
    // Multiply original alpha by shadowAlpha for final opacity
    return vec4(0.0, 0.0, 0.0, swayedColor.a * shadowAlpha);
}
